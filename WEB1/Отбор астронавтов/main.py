...
@app.route('/carousel')
def carousel():
    carousel_path = 'static/img/carousel'
    if not os.path.exists(carousel_path):
        os.makedirs(carousel_path)
    carousel_images = []
    if os.path.exists(carousel_path):
        carousel_images = [f for f in os.listdir(carousel_path)]
    indicators = ""
    slides = ""
    for i, img in enumerate(carousel_images):
        active = "active" if i == 0 else ""
        img_url = url_for('static', filename=f'img/carousel/{img}')
        indicators += f'''
            <button type="button" data-bs-target="#marsCarousel" data-bs-slide-to="{i}" class="{active}" aria-current="{str(i==0).lower()}" aria-label="Slide {i+1}"></button>'''
        slides += f'''
            <div class="carousel-item {active}">
                <img src="{img_url}" class="d-block w-100" alt="Марс {i+1}" style="max-height: 500px; object-fit: cover;">
            </div>'''
    return f'''<!doctype html>
                <html lang="en">
                  <head>
